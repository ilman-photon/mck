            ...
            <div role="main" className="faq-container container flex pt-6 lg:pt-72 flex-col mx-auto px-0  mb-6 lg:mb-18">
                ...
                <div className="grid lg:grid-cols-2 lg:gap-4 mt-6">
                    <div className="rounded-lg shade-blue-border overflow-hidden faq-box">
                        <h3 className="shade-blue-bg text-mckblue text-sofia-bold font-extrabold lg:text-lg text-base py-3 px-4" id="faq_label_008">{DOMPurify.sanitize(faqData?.whatYouCanDoHeading?.value)}</h3>
                        <div className="lg:p-5 p-4 text-mckblue shade-blue-border-top lg:text-lg text-base text-sofia-reg underline" id="faq_label_009" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(faqData?.whatYouCanDoDeatils?.value), }}>
                        </div>
                    </div>
                    <div className="mt-6 lg:mt-0">
                        <div className="rounded-lg shade-blue-border overflow-hidden faq-box">
                            <h3 className="shade-blue-bg text-mckblue text-sofia-bold font-extrabold lg:text-lg text-base py-3 px-4" id="faq_label_010">{DOMPurify.sanitize(faqData?.speaktoAPharmacistHeading?.value)}</h3>
                            <div className="lg:p-5 p-4 text-mckblue shade-blue-border-top lg:text-lg text-base text-sofia-reg underline" id="faq_label_011" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(faqData?.speaktoAPharmacistDetail?.value), }}>
                            </div>
                        </div>
                        <div className="rounded-lg shade-blue-border overflow-hidden mt-6 faq-box">
                            <h3 className="shade-blue-bg text-mckblue text-sofia-bold font-extrabold lg:text-lg text-base py-3 px-4" id="faq_label_012">{DOMPurify.sanitize(faqData?.sendFeedBackHeading?.value)}</h3>
                            <div className="lg:pt-5 lg:pl-5 lg:pr-5 pt-4 pr-4 pl-4 pb-0 text-mckblue shade-blue-border-top lg:text-lg text-base text-sofia-reg" id="faq_label_013" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(faqData?.sendFeedBackDetails?.value), }}>    
                            </div>
                        </div>
                    </div>
                </div>
