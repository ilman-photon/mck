import { type ActiveFiltersDataResponse,type ProductFilter } from "@/components/product_list/Model/ProdutAPI";
import axiosInstance from "@/utils/axiosInstance";
import { create } from "zustand";

interface Controller{
    fetchProductFilterList: () => Promise<void>
    fetchRecommendedProductData: () => Promise<void>
    ...
    productSum:number
    fetchAllProductList:(arr?:any) => Promise<void>
    isLoading:boolean
...
export const useHealthNeedsStore = create<Controller>((set,get) => ({
    fetchProductFilterList: async () => {
      ...
      fetchAllProductList:async (categoryArrayList) => {
        set({isLoading:true})
         await axiosInstance.get(
          ...
          set({
            ...
            productQueriedData:transformedArray,
            isLoading:false
          })
      ...
      fetchRecommendedProductData: async () => {
        ...
        if(callApiRecommendCategoryData.data && callApiRecommendCategoryData.status === 200){
            ...
            callApiRecommendCategoryData?.data?.[0]?.contentArea?.expandedValue?.filter(
            ...
            set({ 
                recommendProductData:response,
                customerBackgroundColor:healthNeedsCategoriesList[0].backgroundColorCode?.value,
                productCategoryDataList:productCategoryDataList,
                healthNeedsCategory:healthNeedsCategoriesListData,
                isLoading:false
            })
        }
      },
