...
import {
  customAdd,
  customUniqueElementArray,
  deleteMultipleElements,
  extractMainCategoryId,
  extractMainCategoryName,
  selectedProductType,
  selectedProductType_,
} from "../global/CommonUtil";
...
const HealthNeedFilter = ({
  ...
  const handleCheckBox = (
    ...
    if (e.target.checked) {
      ...
      const subIds = group?.find(
        (c: any) => c.mainCatId === categoryId
      )?.subCateIds; //?.splice(index, 1)
      const idx = subIds?.indexOf(filter);
      const a = subIds?.splice(idx, 1);
      if (group?.subCateIds?.length == 0) {
        setGroup([]);
      }
      setActiveFilter(
  ...
  useEffect(() => {
    const selectedProductType = productCategoryData?.find(
      (a: any) => a.mainCategory?.value[0]?.id === mainCategoryId
    );
    const subCategoryValues = Array.from(
      new Set([selectedProductType?.subCategory?.value])
    ).filter(Boolean);
    if (selectedFilterItems[mainCategoryId]) {
      selectedFilterItems[mainCategoryId].items = customUniqueElementArray(
        selectedFilterItems[mainCategoryId]?.items
      );
    }
    if (
      customUniqueElementArray(selectedFilterItems[mainCategoryId]?.items)
        ?.length === subCategoryValues.flat().length
    ) {
      if (selectedFilterItems[mainCategoryId]) {
        const isAllAreChecked = selectedFilterItems[mainCategoryId]
          .map((sub_category: any) => sub_category.checked)
          .filter(Boolean);
  ...
  useEffect(() => {
    const currentCategory = group?.find(
      (gr: any) => gr.mainCatId === mainCategoryId
    );
    if (currentCategory && Object.keys(currentCategory).length > 0) {
      const [value, count, name] = selectedProductType(
        productCategoryData,
        mainCategoryId
      );
      if (count === currentCategory?.subCateIds?.length) {
        setMainCatNames([...mainCatNames, name]);
        const catIds = [...mainCatNames, name]?.map((mcn: any) =>
          extractMainCategoryId(productCategoryData, mcn)
        );
        const process = catIds?.map((cId: any) =>
          selectedProductType_(productCategoryData, cId)
        );
        const subCateNames = process.flat()?.map((p: any) => p.name);
        const onlyOtherSubCateIds = deleteMultipleElements(
          activeFilter,
          subCateNames
        );
        setActiveFilter([
          ...Array.from(new Set([...mainCatNames, name])).filter(Boolean),
          ...onlyOtherSubCateIds,
        ]);
      } else {
        if (mainCategoryId === currentCategory?.mainCatId) {
          if (mainCatNames?.includes(name)) {
            const findIndex = mainCatNames?.findIndex(
              (item: any) => item === name
            );
            const findMainCatNameIndex = activeFilter
              ?.flat()
              .findIndex((item: any) => item === name);
            mainCatNames.splice(findIndex, 1);
            activeFilter.splice(findMainCatNameIndex, 1);
            const common = Array.from(
              new Set([...mainCatNames, ...activeFilter])
            );
            const findCommonIndex = common
              ?.flat()
              .findIndex((item: any) => item === name);
            if (findCommonIndex >= 0) {
              common?.splice(findCommonIndex, 1);
            }
            const catIds = mainCatNames?.map((mcn: any) =>
              extractMainCategoryId(productCategoryData, mcn)
            );
            const process = catIds?.map((cId: any) =>
              selectedProductType_(productCategoryData, cId)
            ...
            setActiveFilter(
              Array.from(
                new Set([
                  ...common1.filter((a: any) => !Array.isArray(a)),
                  ...currentCategory?.subCateIds,
                ])
              )
            );
          }
              ...
              activeFilter?.map((item: any, index: number) => {
                return (
                  <div
                    ...
                    key={item}
                  >
                    {extractMainCategoryName(productCategoryData)?.includes(
                      item
                    )
                      ? `${item}-All`
                      : `${item}`}
