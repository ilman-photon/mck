            ...
            <div className="four-oh-four why-FT container flex flex-col mx-auto px-0 lg:pt-0 lg:px-0">
                ...
                <div className="relative lg:mb-12 mb-6">
                    ...
                    <div className="lg:absolute top-0 left-0 right-0 bottom-0 bg-transparent-left lg:pr-11 lg:pl-6 w-540 flex flex-col justify-center lg:items-end pt-8 lg:pt-0">
                        ...
                        <button className="p-3 uppercase rounded-lg blue-bg text-white text-sofia-bold font-extrabold text-base float-right lg:block hidden" id="wft_btn_007" aria-label={whyFTData?.contentArea?.expandedValue[2]?.buttonText?.value}>{whyFTData?.contentArea?.expandedValue[2]?.buttonText?.value}</button>
                    </div>
                </div>
                ...
                <div className="relative lg:mb-12 mb-6">
                    ...
                    <div className="lg:absolute top-0 right-0 bottom-0 bg-transparent-right lg:pr-11 lg:pl-6 w-540 float-right flex flex-col justify-center lg:items-end pt-8 lg:pt-0">
                        <h2 className="text-mckblue text-gtl-med heading font-medium lg:pb-6 pb-4" id="wft_label_010" tabIndex={0}>{whyFTData?.contentArea?.expandedValue[4]?.name}</h2>
                        <p className="pb-6 text-sofia-reg lg:text-lg font-normal text-mcknormalgrey text-base" id="wft_label_011" tabIndex={0} dangerouslySetInnerHTML={{__html: whyFTData?.contentArea?.expandedValue[4]?.description?.value,}}></p>
                        <button className="p-3 uppercase rounded-lg blue-bg text-white text-sofia-bold font-extrabold text-base float-right lg:block hidden" id="wft_btn_012" aria-label={whyFTData?.contentArea?.expandedValue[4]?.buttonText?.value}>{whyFTData?.contentArea?.expandedValue[4]?.buttonText?.value}</button>
                    </div>
                </div>
                ...
                <div className="w-full grid lg:flex items-center">
                    ...
                    <div className="lg:w-3/6 lg:pl-18 lg:pr-12 lg:pt-0 pt-4">
                        ...
                        <button className="p-3 uppercase rounded-lg blue-bg text-white text-sofia-bold font-extrabold text-base float-right lg:block hidden" id="wft_btn_019" aria-label={whyFTData?.contentArea?.expandedValue[6]?.buttonText?.value}>{whyFTData?.contentArea?.expandedValue[6]?.buttonText?.value}</button>
                    </div>
                </div>
            </div>
        </>
