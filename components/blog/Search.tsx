...
const SearchComponent: React.FC<CatogaryComponentProps> = ({ placeholder, searchText, ActiveSearch, handleResponse, handleClose }) => {
    ...
    return (
        <>
            ...
            />
            {ActiveSearch ? <svg onClick={fetchSearchBlog} tabIndex={0} className="absolute top-4 right-4 z-8 cursor-pointer" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            </svg> : <svg tabIndex={0} onClick={() => { handleClose('List'), setsearch('') }} className="absolute top-4 right-4 z-8 cursor-pointer" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
