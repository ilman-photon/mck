...
const BlogDetailsComponent = () => {
    ...
    return (
        <>{loading ?
            ...
            (<div className='container blog-details-container w-full lg:px-0  lg:py-72 lg:pb-0 p-4 pt-6  mx-auto ' id="carouselExampleCaptions" >
                <div className='col-start-1 col-end-7 rounded-lg overflow-hidden'
                    style={{ display: BlogInfo.image.expandedValue.url ? 'block' : 'none' }}
                >
                    <ImageComponent
                    ...
                    <div className='lg:w-966 lg:col-span-2 col-start-1 col-end-7 lg:pl-4'>
                        <article className='mb-6'>
                            <div className='content lg:p-6 pb-0 p-4 px-0'>
                                <h1 className='lg:text-54  lg:leading-61  max-[576px]:leading-9 text-32 text-gtl-med text-mckblue lg:pb-6 text-center' id='blog-link-001'>{BlogInfo.title?.value}</h1>
                                <div className='grid lg:grid-cols-2 lg:gap-4 lg:pt-0 pt-4 lg:pb-6 pb-4'>
                                    <div className='lg:pb-0 pb-4'>
                                        <span className={`text-mckblue text-sofia-reg font-normal text-base pr-2 border-solid ${BlogInfo.readMinute?.value ? 'shade-grey-right-border' : ''}`} id='blog-label-001'>{GetTime(BlogInfo.startPublish)}</span>
                                        <span className={`text-mckblue text-sofia-reg font-normal text-base px-2 border-solid ${false ? 'shade-grey-right-border' : ''}`} id='blog-label-002'>{BlogInfo.readMinute?.value}</span>
                                ...
                                <div className="text-sofia-reg text-color text-lg leading-27" dangerouslySetInnerHTML={{ __html: BlogInfo.description.value }}></div>
                            </div>
                        </article>
